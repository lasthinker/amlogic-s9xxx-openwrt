...
- name: Build OpenWrt firmware
  ...
  run: |
    ...
    sudo chmod +x make
    sudo ./make -d -b s905x3_s905x2_s905x_s905w_s905d_s922x_s912 -k 5.4.144_5.10.62_5.14.1
...
- name: Package Armvirt as OpenWrt
  ...
  with:
    ...
    amlogic_kernel: 5.4.144_5.10.62_5.14.1
    auto_kernel: true
    amlogic_size: 1024
```
...
| amlogic_kernel         | 5.4.144_5.14.1         | Set the kernel version，lasthinker's [kernel](https://github.com/lasthinker/flippy-kernel/tree/main/library) library contains many original kernels of `Flippy`, you can view and choose to specify. |
...
- `sudo ./make -d -b s905x3_s905d -k 5.4.144_5.14.1`: Use the default configuration, specify multiple cores, and multiple firmware for compilation. use `_` to connect.
...
- `sudo ./make -d -k 5.4.144_5.14.1`: Use the default configuration. Specify multiple cores, use `_` to connect.
- `sudo ./make -d -k 5.4.144_5.14.1 -a true`: Use the default configuration. Specify multiple cores, use `_` to connect. Auto update to the latest kernel of the same series.
...
| -k | Kernel | Specify the kernel type. Write the kernel name individually such as `-k 5.4.132` . Multiple cores use `_` connection such as `-k 5.4.144_5.14.1` [kernel](https://github.com/lasthinker/flippy-kernel/tree/main/library). |
| -a | AutoKernel | Set whether to automatically adopt the latest version of the kernel of the same series. When it is `true`, it will automatically find in the kernel library whether there is an updated version of the kernel specified in `-k` such as 5.14.1. If there is the latest version of 5.14 same series, such as 5.14.1 and later, it will automatically Replace with the latest version. When set to `false`, the specified version of the kernel will be compiled. Default value: `true` |
